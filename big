...
for ($i=0; $i < $jumlah; $i++) { 
    ...
    while ($regis) {
        ...
        $getEmail = file_get_contents('https://api.hax0r.id/bigtoken/regis.php?reff='.$reff.'&code='.$code);
    ...
    echo " - Check email: ";
    while ($pesan) {
        $getURL = file_get_contents('https://api.hax0r.id/bigtoken/inbox.php?email='.$email.'&code='.$code);
    ...
    sleep(1);
    $getVerif = file_get_contents('https://api.hax0r.id/bigtoken/verif.php?url='.$verifURL.'&code='.$code);
